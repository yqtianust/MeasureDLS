...
class TestTensorFlow(unittest.TestCase):
    ...
    def test_imagenet_resnet50_v2(self):
        ...
        neuron_coverage = NeuronCoverage(threshold=0.4)
        measure_model.intermediate_layer_outputs(x_preprocessed, [neuron_coverage.update])
        self.assertAlmostEqual(neuron_coverage.neuron_coverage, 0.47886641839648636)
    ...
    def test_imagenet_mobilenet_v2(self):
        ...
        neuron_coverage = NeuronCoverage(threshold=0.4)
        measure_model.intermediate_layer_outputs(x_preprocessed, [neuron_coverage.update])
        self.assertAlmostEqual(neuron_coverage.neuron_coverage, 0.1660512517539325)
    ...
    def test_cifar10_simple(self):
        ...
        self.assertAlmostEqual(accuracy_top_1.accuracy, 0.3271)
        self.assertAlmostEqual(accuracy_top_5.accuracy, 0.8202)
        end = time.time()
        print('accuracy time', end - start)

        neuron_coverage = NeuronCoverage(threshold=0.6)
        measure_model.intermediate_layer_outputs(x, [neuron_coverage.update])
        self.assertAlmostEqual(neuron_coverage.neuron_coverage, 0.1581196581196581)
    ...
    def test_mnist_simple(self):
        ...
        accuracy_top_5 = Accuracy(k=5)
        measure_model.predict(dataset, [accuracy_top_1.update, accuracy_top_5.update])
        self.assertAlmostEqual(accuracy_top_1.accuracy, 0.9377)
        self.assertAlmostEqual(accuracy_top_5.accuracy, 0.9972)
        end = time.time()
        print('accuracy time', end - start)

        neuron_coverage = NeuronCoverage(threshold=0.4)
        measure_model.intermediate_layer_outputs(x, [neuron_coverage.update])
        self.assertAlmostEqual(neuron_coverage.neuron_coverage, 0.3035398230088496)
